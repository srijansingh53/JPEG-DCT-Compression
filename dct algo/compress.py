...
def Compress(filepath,rate):
    ...
    cv2.imwrite('quantized.bmp',padded_img)
    ...
    cv2.imwrite("compressed_image.bmp",np.uint8(comp_img))
    output_path = str(pathlib.Path(__file__).parent.absolute())+'\compressed_image.bmp'
    # print(output_path)
    return output_path